...
bool MAX30105::begin(I2C_HandleTypeDef* wirePort, TIM_HandleTypeDef* ptimer, uint32_t i2cSpeed, uint8_t i2caddr) {

  _i2cPort = wirePort; //Grab which port the user wants us to use

  _i2caddr = i2caddr;
  _i2c_rd_addr = _i2caddr << 1 | 0x01;
  _i2c_wr_addr = _i2caddr << 1;
...
uint16_t MAX30105::millis(){
	uint16_t timer_val = __HAL_TIM_GET_COUNTER(timer);
...
uint16_t MAX30105::check(void)
{
  ...
  if (readPointer != writePointer)
  {
	...
	i2c_buffer[0] = MAX30105_FIFODATA;
	HAL_I2C_Master_Transmit(_i2cPort, _i2c_wr_addr, i2c_buffer, 1, 10);
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      HAL_I2C_Master_Receive(_i2cPort, _i2c_rd_addr, &i2c_buffer[1], toGet, 10);
...
uint8_t MAX30105::readRegister8(uint8_t address, uint8_t reg) {
  return i2c_read_from_reg(reg, 1);
}

void MAX30105::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
	i2c_write_to_reg(reg, value);
}

void MAX30105::i2c_write_to_reg(uint8_t reg_addr, uint8_t data){
	i2c_buffer[0] = reg_addr;
	i2c_buffer[1] = data;
	HAL_I2C_Master_Transmit(_i2cPort, _i2c_wr_addr, i2c_buffer, 2, 10);
}

uint8_t MAX30105::i2c_read_from_reg(uint8_t reg_addr, uint8_t num_read_bytes){
	HAL_StatusTypeDef ret;

	i2c_buffer[0] = reg_addr;
	i2c_buffer[1] = 0;
	ret = HAL_I2C_Master_Transmit(_i2cPort, _i2c_wr_addr, i2c_buffer, 1, 10);

	if (ret != HAL_OK){
		return 0;
	}

	ret = HAL_I2C_Master_Receive(_i2cPort, _i2c_rd_addr, &i2c_buffer[1], num_read_bytes, 10);
