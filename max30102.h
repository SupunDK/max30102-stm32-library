...
class MAX30105 {
 public:
  ...
  bool begin(I2C_HandleTypeDef* wirePort, TIM_HandleTypeDef* ptimer, uint32_t i2cSpeed = I2C_STANDARD_RATE, uint8_t i2caddr = MAX30105_ADDRESS);
  ...
  void i2c_write_to_reg(uint8_t reg_addr, uint8_t data);
  uint8_t i2c_read_from_reg(uint8_t reg_addr, uint8_t num_read_bytes);
  uint16_t millis();
 private:
  I2C_HandleTypeDef* _i2cPort; //The generic connection to user's chosen I2C hardware
  uint8_t _i2caddr;
  uint8_t _i2c_rd_addr;
  uint8_t _i2c_wr_addr;
  uint8_t i2c_buffer[6*32];
